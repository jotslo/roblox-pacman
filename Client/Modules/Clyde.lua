...
local function Pathfind()
	if DesiredDir == Direction then
		if Mode == "Chase" then
			OverallTarget = Source.GetPacmanPos:Invoke()
		end
...
function Clyde:Run()
	...
	coroutine.wrap(function()
		...
		wait(1)
		MoveScript:Update(Ghost, "Left", "Left", Vector2.new(3, 3))
		while Running do
			wait(0.04)
			local NewDir
...
function Clyde:Reset(CanContinue)
	Running = false
	coroutine.wrap(function()
		wait(1)
		Ghost.Position = UDim2.new(0.533, 0, 0.427, 0)
		Direction = "Left"
		MostRecentDir = "Left"
		DesiredDir = "Down"
